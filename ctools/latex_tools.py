...
# with modifications
def parse_nested_braces(string: str) -> Tuple[int, str]:
...
def extract_pages_from_aux(auxfile: str) -> List:
...
def delete_temp_files(latex_source: str, verbose: bool = False) -> None:
...
TEXINPUTS='TEXINPUTS'
def run_latex(pathname: str, repeat: int = 1, start_run: int = 1, delete_tempfiles: bool =  False,
              texinputs: str = None,
              callback_aux=None, callback_log=None, ignore_ret: bool = False,
              chdir: bool = True, verbose: bool = False) -> Tuple[str, int]:
...
def extract_pdf_pages(target: str, source: str, pagelist: str = "-") -> None:
...
def inspect_json_all_pages_have_same_orientation(info: Dict):
...
def inspect_pdf_latex(pdf_fname: str, texinputs: str = None) -> Dict:
...
def inspect_pdf_pypdf(pdf_fname: str) -> Dict:
...
def inspect_pdf(pdf_fname: str, texinputs: str = None) -> Dict:
    try:
...
def count_pdf_pages_pypdf(pdf_fname: str) -> int:
    import PyPDF2
    pdf = PyPDF2.PdfFileReader(open(pdf_fname, "rb"), strict=False)
    return pdf.getNumPages()


def count_pdf_pages(pdf_fname: str) -> int:
