...
def aws_emr_cmd(cmd: str, retries: int = MAX_RETRIES, decode: bool = True):
...
def list_clusters(*, state=None) -> Dict:
...
def describe_cluster(clusterId: str):
    data = aws_emr_cmd(['describe-cluster', '--cluster', clusterId])
    return data['Cluster']


def list_instances(clusterId: str = None):
...
def complete_cluster_info(workers=DEFAULT_WORKERS, terminated: bool = False) -> List:
