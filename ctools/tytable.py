...
def scalenum(v: Union[int, str], minscale: int = 0) -> str:
...
def latex_var(name: str, value, desc: str = None, xspace: bool = True) -> str:
...
def text_var(name: str, value, desc: str = None) -> str:
    """Create a variable NAME with a given VALUE.
    Primarily for output to LaTeX.
    Returns a string."""
...
class Row:
    ...
    def __init__(self, data: List, annotations: List = None) -> None:
        if annotations:
...
class Raw(Row):
    def __init__(self, rawdata, *, ncols: int) -> None:
        self.data  = rawdata
        self.ncols_ = ncols
...
class ttable:
    ...
    def set_fontsize(self, ft: int) -> None:
        self.fontsize = ft

    def add_option(self, o) -> None:
        self.options.add(o)

    def set_option(self, o) -> None:
        self.options.add(o)
    ...
    def set_col_alignment(self, col: int, align: str) -> None:
        self.col_alignment[col] = align

    def set_col_alignmnets(self, fmt: str)  -> None:
        col = 0
    ...
    def set_col_totals(self, totals) -> None:
        self.col_totals = totals

    def set_col_fmt(self, col: int, fmt: str) -> None:
    ...
    def add_head(self, values, annotations: List = None) -> None:
    ...
    def add_subhead(self, values, annotations: List = None) -> None:
        self.data.append(Head(values, annotations=annotations))

    def add_data(self, values, annotations: List = None) -> None:
    ...
    def add_raw(self, val, *, ncols: int) -> None:
    ...
    def format_cell(self, value, col_number: int) -> Tuple[str, str]:
    ...
    def col_formatted_width(self, col_num: int) -> int:
    ...
    def typeset_cell(self, formatted_value: str, col_number: int) -> str:
    ...
    def typeset_row(self, row: List, html_delim: str = 'td') -> str:
    ...
    def calculate_col_formatted_widths(self) -> List[int]:
    ...
    def should_omit_row(self, row) -> bool:
        for (a, b) in self.omit_row:
            if row[a] == b:
                return True
        return False

    def compute_and_add_col_totals(self) -> None:
    ...
    ################################################################
    def typeset_headings(self) -> List:
        #
    ...
    def typeset(self, *, mode: str = None, option: List = None, out=None) -> str:
    ...
    def save_table(self, fname: str, mode: str = LATEX, option: List = None) -> None:
        with open(fname, "w") as f:
            f.write(self.typeset(mode=mode, option=option))

    def add_sql(self, db, stmt: str, headings: str = None, footnote: bool = False) -> None:
        if footnote:
            self.footnote = stmt
        cur = db.cursor()
        try:
