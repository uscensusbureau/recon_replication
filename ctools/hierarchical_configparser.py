...
def getOption(line: str) -> Union[str, None]:
    """
...
class HCP:
    ...
    def read(self, filename: str, *, onlySection: str = None) -> None:
        """
    ...
    def asString(self) -> str:
        ...
        for (section, lines) in self.sections.items():
            if len(lines) > 0:
                m = SECTION_RE.search(lines[0]) # make sections lowercase
                if m and m.group(1).lower() != DEFAULT_SECTION:
...
class HierarchicalConfigParser(ConfigParser):
    ...
    def read(self, filenames: Union[str, List[str]], validate: bool = False) -> None:
        """
        ...
        :param validate: Validate the read files.
        """
    ...
    def write(self, fileobject, **kwargs) -> None:
