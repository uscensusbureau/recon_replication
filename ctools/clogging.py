...
def applicationIdFromEnvironment() -> str:
...
def add_argument(parser: ArgumentParser, *, loglevel_default: str = 'INFO') -> None:
...
#
# https://stackoverflow.com/questions/52950147/sysloghandler-messages-grouped-on-one-line-on-remote-server
def setup_syslog(facility: str = logging.handlers.SysLogHandler.LOG_LOCAL1,
                 syslog_address=None,
                 syslog_format: str = YEAR + " " + SYSLOG_FORMAT,
                 use_tcp: bool = False) -> None:
    global added_syslog
    if not added_syslog:
        for try_tcp in [use_tcp, False]:
            ...
            if try_tcp:
                ...
                # From stackoverflow 52950147 above:
                #
                # This will add a line break to the message before it
                # is 'emitted' which ensures that the messages are
                # split up over multiple lines, see
            ...
            try:
                ...
                if try_tcp:
                    warnings.warn("sysload falling back to UDP")
                    continue
                warnings.warn("Connection refused and use_tcp is False???")
                raise
            ...
            added_syslog = True
            return              # don't retry if we were successful
        # end of for loop ; never reaches here
    # end of if not added_syslog; never reaches here
...
def setup(level = logging.INFO,
          ...
          syslog_address=None,
          filename: str = None,
          facility: str = logging.handlers.SysLogHandler.LOG_LOCAL1,
          log_format: str = LOG_FORMAT,
          syslog_format: str = SYSLOG_FORMAT) -> None:
