...
def sql_InternalError() -> RuntimeError:
    try:
...
def timet_iso(t: float = time.time()) -> str:
...
class DBSQL(ABC):
    def __init__(self, dicts: bool = True, debug: bool = False) -> None:
        self.dicts = dicts
        self.debug = debug
    ...
    def execute(self, cmd: str, *args, debug: bool = False, **kwargs):
    ...
    def create_schema(self, schema: str, *, debug: bool = False) -> None:
...
class DBSqlite3(DBSQL):
    ...
    def set_cache_bytes(self, b: int) -> None:
    ...
    def csfr(self, auth, cmd: str, vals=[], quiet: bool = True, rowcount=None, time_zone=None,
             get_column_names=None, asDicts=False, debug: bool = False, cache=True):
        assert auth is None
...
connection. """
    ...
    def __init__(self, *, host, database, user: str, password: str, debug: bool = False) -> None:
        self.host     = host
    ...
    @staticmethod
    def GetBashEnv(filename: str) -> Dict:
    ...
    @staticmethod
    def FromConfig(section: str, debug: bool = None):
...
class DBMySQL(DBSQL):
    ...
    def __init__(self, auth, *args, **kwargs) -> None:
        super().__init__(*args, **kwargs)
        self.auth  = auth
    ...
    @staticmethod
    def csfr(auth, cmd: str, vals=None, quiet: bool = True, rowcount: int =  None, time_zone=None,
             setup: str = None, setup_vals=(), get_column_names=None, asDicts: bool = False,
             debug: bool = False, dry_run: bool = False, cache=True, nolog=[]):
        ...
        for i in range(1, RETRIES):
            try:
                ...
                # This is required becuase some errors can be retried and some can't.
                # I wasn't able to find a consistent list.
                if e.args[0] in (1044,1045):  # access denied
                    print(f"Access denied: auth:{auth}", file=sys.stderr)
                    raise(e)
                elif e.args[0]==1054:  # invalid column
                    print(f"Invalid Column in SQL: {cmd}",file = sys.stderr)
                    raise(e)
                elif e.args[0]==1066:  # non-unique table alias
                    print(f"Non-unique table alias in SQL: {cmd}",file = sys.stderr)
                    raise(e)
                elif e.args[0]==1049:
                    print(f"Unknown database in SQL: {cmd}",file=sys.stderr)
                    raise(e)
                if i>1:
                    logging.warning(e)
                ...
                auth.cache_clear()
                pass
            # These may no longer be required; it's not clear to me if InternalError and BlockingIOError are subclasses of OperationalError
    ...
    @staticmethod
    def table_columns(auth, table_name: str):
