...
def valid_sql_name(name: str) -> bool:
    for ch in name:
...
def vtype_for_numpy_type(t) -> Dict:
    import numpy
    try:
...
def sql_type_for_python_value(val) -> str:
...
def sql_parse_create(stmt: str) -> Dict:
    if "--" in stmt:
...
def convertValue(val: str, vtype: str = None):
