...
def get_docx_table(path: str) -> List:
    """
...
def get_text_for_table(table) -> List:
    """
    Find the table inside the .docx file and return it in an array
    """
    rows = []
...
def get_table_rows(tbl) -> List:
    return [row for row in tbl.iter(TR)]


def get_row_cells(row) -> List:
    return [cell for cell in row.iter(TC)]


def get_docx_text(path: str) -> List:
    """
