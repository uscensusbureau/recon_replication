...
@functools.lru_cache()
def get_config(pathname: str = None, filename: str = None):
...
def setup_logging(*, config: ConfigParser, level: str) -> None:
...
def logging_exit() -> None:
    if hasattr(sys, 'last_value'):
        logging.error(sys.last_value)


def argparse_add_logging(parser: argparse.ArgumentParser) -> None:
...
def dpath_exists(path: str) -> bool:
...
def dopen(path: str, mode: str = 'r', encoding: str = 'utf-8'):
