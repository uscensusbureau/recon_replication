...
class Renderer:
    ...
    @staticmethod
    def write_tag_begin(doc, f) -> bool:
        f.write(f'{repr(doc)}\n')
        return True

    # Default write_text just writes the text
    @staticmethod
    def write_text(doc, f, text: str = None) -> bool:
        if text is None:
            text = doc.text
        if text is not None:
            f.write(text)
    ...
    @staticmethod
    def write_tag_end(doc, f) -> bool:
        f.write(f'--- end repr(doc) ---\n')
        return True

    # By default, we write the tail to output
    @staticmethod
    def write_tail(doc, f, tail: str = None) -> bool:
        if tail is None:
            tail = doc.tail
        if tail is not None:
            f.write(tail)
...
class LatexRenderer(Renderer):
    ...
    @staticmethod
    def write_text(doc, f, text: str = None) -> None:
        if text is None:
            text = doc.text
        if text is not None:
    ...
    @staticmethod
    def write_tail(doc, f, tail: str = None) -> None:
        if tail is None:
            tail = doc.tail
        if tail is not None:
            if hasattr(doc, 'option') and doc.option(OPTION_NO_ESCAPE):
...
def render(doc, f, format: str = FORMAT_HTML) -> bool:
    """
...
class TyTag(ET.Element):
    ...
    def asString(self, format: str = 'html') -> str:
    ...
    def clear_option(self, option: str):
        """@param option is a string that is added to the 'option' attrib.
        They are separated by commas"""
        options = self.options_as_set()
    ...
    def set_attrib(self, newAttribs: Dict):
    ...
    def add_tag_elems(self, tag: str, elems: List = [], attrib: Dict = {}, position: int = -1,
                      id: str = None, className: str = None):
        """
    ...
    def add_tag(self, tag: str, attrib: Dict = {}, position: int = -1, **kwargs):
        return self.add_tag_elems(tag, elems=[], attrib=attrib, position=position, **kwargs)

    def add_tag_text(self, tag: str, text: str = '', attrib: Dict = {}, position: int = -1, **kwargs):
    ...
    def append_image(self, buf: io.BytesIO, *, format: str) -> None:
    ...
    def append_matplotlib(self, fig, *, format: str = "png", **kwargs) -> None:
        buf = io.BytesIO()
    ...
    def ul(self, text: str = '', **kwargs):
        """Add a UL"""
        return self.body_().add_tag_text(TAG_UL, text, **kwargs)

    def li(self, text: str = '', **kwargs):
        """Add a LI"""
        return self.body_().add_tag_text(TAG_UL, text, **kwargs)

    def span(self, text: str = '', **kwargs):
        return self.body_().add_tag_text(TAG_SPAN, text, **kwargs)


class EmbeddedImageTag(TyTag):
    def __init__(self, buf: io.BytesIO, *, format: str, alt: str = "") -> None:
    ...
    def custom_renderer(self, f, alt: str = "", format: str = FORMAT_HTML) -> None:
...
class tydoc(TyTag):
    ...
    def write_tag_begin(self, f, format: str = None) -> bool:
    ...
    @staticmethod
    def write_tag_end(f, format: str = None) -> bool:
        if format == FORMAT_LATEX:
    ...
    def insert_toc(self, level: int = 3) -> None:
...
################################################################
class X_TOC(TyTag):
    def __init__(self, attrib: Dict = {}, **extra) -> None:
    ...
    @staticmethod
    def custom_renderer(f, format: str = FORMAT_HTML) -> bool:
    ...
    @staticmethod
    def write_tag_begin(f, format: str = FORMAT_HTML) -> bool:
        if format == FORMAT_HTML:
            # Output nothing
            return True
        return False
    ...
    @staticmethod
    def write_tag_end(f, format: str = FORMAT_HTML)-> bool:
        if format == FORMAT_HTML:
            # Output nothing
            return True
        return False
...
class jsonTable(TyTag):
    ...
    def custom_renderer(self, f, format: str = FORMAT_HTML) -> bool:
        return self.custom_renderer_json(f)

    def custom_renderer_json(self, f) -> bool:
        """
    ...
    def add_row(self, where: str, cells, row_attrib: Dict = {}, row_auto_id: str = None) -> None:
        """
    ...
    def add_row_values(self, where: str, tags: Union[str, List[str]], values: List, *, cell_attribs: List = None,
                       row_attrib: List = None, row_auto_id: str = None) -> None:
        """
    ...
    def add_head(self, values: List, row_attrib: List = None, cell_attribs: List = None, col_auto_ids: List = None,
                 row_auto_id: str = "head") -> None:
    ...
    def add_data(self, values: List, row_attrib: List = None, cell_attribs: List = None, row_auto_id: str = None) -> None:
        self.add_row_values(TAG_TBODY, 'TD', values, row_attrib=row_attrib, cell_attribs=cell_attribs,
                            row_auto_id=row_auto_id)

    def add_foot(self, values: List, row_attrib: List = None, cell_attribs: List = None) -> None:
        self.add_row_values(TAG_TFOOT, 'TD', values, row_attrib=row_attrib, cell_attribs=cell_attribs,
                            row_auto_id="foot")

    def add_data_array(self, rows: List) -> None:
        for row in rows:
            self.add_data(row)

    def make_cell(self, tag: str, value, attrib: Dict):
        """
...
class tytable(TyTag):
    ...
    def custom_renderer_latex(self, f) -> bool:
    ...
    def custom_renderer_csv(self, f) -> bool:
    ...
    def custom_renderer_md(self, f) -> bool:
        """
    ...
    def latex_cell_text(self, cell) -> str:
        if self.option(OPTION_NO_ESCAPE):
            return cell.text
        else:
            return latex_escape(cell.text)

    def render_latex_table_row(self, f, tr) -> None:
    ...
    def render_latex_table_head(self, f) -> None:
    ...
    def render_latex_table_body(self, f) -> None:
        """Render the rows that were not added with add_head() command"""
        for tr in self.findall("./TBODY/TR"):
            self.render_latex_table_row(f, tr)

    def render_latex_table_foot(self, f) -> None:
    ...
    def set_latex_colspec(self, latex_colspec: str) -> None:
    ...
    def add_row(self, where: str, cells: List, row_attrib: Dict = {}, row_auto_id: str = None) -> None:
        """
        Add a row of cells to the table.
        You probably want to call add_head(), add_data() or add_foot().
    ...
    def add_row_values(self, where: str, tags: Union[str, List], values: List, *, cell_attribs: List = None,
                       row_attrib: List = None, row_auto_id: str = None) -> None:
        """
        Create a row of cells and add it to the table. This is called by add_head, add_data, or add_foot.
    ...
    def add_data_array(self, rows) -> None:
        for row in rows:
            self.add_data(row)
...
def a(*text, href=None, attrib: Dict = {}, **kwargs):
    """Return an anchor"""
...
def th(*text, attrib: Dict = {}, **kwargs):
...
# Useful for jupyter
#
def jupyter_display_table(val: Union[Dict, List], float_format: str = "{:.5f}") -> None:
    import io
