...
def ex(line: str, desc: Tuple[int, int]) -> str:
    return line[desc[0]-1:desc[0]+desc[1]-1]


def exi(line: str, desc: Tuple[int, int]) -> int:
    return int(ex(line,desc))


def test_geo_fields() -> None:
...
def test_load_geo_file() -> None:
    ...
    geofilename = os.path.join(os.path.dirname(__file__), "../data/2010_pl94/akgeo2010.pl")

    load_file(conn,open(geofilename),decode_geo_line)
